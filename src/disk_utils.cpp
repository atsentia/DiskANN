               ...
               uint64_t warmup_aligned_dim)
{
    ...
    {
        size_t warmup_num_temp = warmup_num;
        diskann::load_aligned_bin<T>(cache_warmup_file, warmup, warmup_num_temp, file_dim, file_aligned_dim);
        warmup_num = warmup_num_temp;
        if (file_dim != warmup_dim || file_aligned_dim != warmup_aligned_dim)
        {
            std::stringstream stream;
            stream << "Mismatched dimensions in sample file. file_dim = " << file_dim
                 ...
                 const std::string &labels_to_medoids_file)
{
    ...
    for (const auto &id_shard : node_shard)
    {
        ...
        if (cur_id < node_id)
        {
            ...
            nnbrs = (uint32_t)(std::min)((uint64_t)final_nhood.size(), (uint64_t)max_degree);
